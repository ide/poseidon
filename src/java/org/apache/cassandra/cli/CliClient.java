...
public class CliClient 
{
	...
	private void printCmdHelp()
	{
		...
		css_.out.println("set <ksp>.<cf>['<key>']['<col0>,<col1>,..,<colN>'] = '<val0>,<val1>,..,<valN>'  ");
		css_.out.println("                                                                    Set columns.");
	...
	private void executeGet(CommonTree ast) throws TException, NotFoundException, InvalidRequestException, UnavailableException, TimedOutException, UnsupportedEncodingException, IllegalAccessException, InstantiationException, ClassNotFoundException
	{
		...
		ConsistencyLevel lvl = ((ast.getChildCount() == 2) ? 
				extractLvl(ast.getChild(1).toString()) 
				: ConsistencyLevel.ONE);

		String[] cols = new String(columnName).trim().split(SEP_PAT);
		if (cols.length > 1) {
			List<ColumnOrSuperColumn> out = thriftClient_.get_slice(tableName, key, new ColumnParent(columnFamily), makeSlicePred(cols), lvl);
			for (ColumnOrSuperColumn c : out)
				css_.out.printf("=> (column=%s, value=%s, timestamp=%d)\n", formatColumnName(tableName, columnFamily, c.column),
						new String(c.column.value, "UTF-8"), c.column.timestamp);
			css_.out.printf("Returned %d results.", out.size());
		} else {
			Column column = thriftClient_.get(tableName, key, createColumnPath(columnFamily, superColumnName, columnName), lvl).column;
	...
	{ 
		...
		assert (ast.getChildCount() <= 3) : "serious parsing error (this is a bug).";
		...
		ConsistencyLevel lvl = ((ast.getChildCount() == 3) ? 
				extractLvl(ast.getChild(2).toString()) 
				: ConsistencyLevel.ONE);

		// do the insert
		String[] columns = new String(columnName).trim().split(SEP_PAT);	
		if (columns.length > 1) {
			String[] vals = value.trim().split(SEP_PAT);
			if (columns.length != vals.length) {
				System.err.println("Number of columns not equal to number of values");
				return;
			}
			thriftClient_.batch_insert(tableName, key, makeCFMap(columnFamily, columns, vals), lvl);
			css_.out.println(columns.length + " values inserted.");
			
		} else {
			thriftClient_.insert(tableName, key, createColumnPath(columnFamily, superColumnName, columnName),
					value.getBytes(), timestampMicros(), lvl);
	...
	private static ConsistencyLevel extractLvl(String lvl) {
		...
		} else if (lvl.equalsIgnoreCase("DCQUORUM")) {
			return ConsistencyLevel.DCQUORUM;
		} else if (lvl.equalsIgnoreCase("DCQUORUMSYNC")) {
			return ConsistencyLevel.DCQUORUMSYNC;
		} else if (lvl.equalsIgnoreCase("ALL")) {
			return ConsistencyLevel.ALL;
		} else if (lvl.equalsIgnoreCase("ANY")) {
			return ConsistencyLevel.ANY;
		} else 
			throw new RuntimeException("attempt to extract invalid consistency level");
	}

	/** Requires (columns.length == fileNames.length) */
	private static Map<String,List<ColumnOrSuperColumn>> makeCFMap(String columnFamily, String[] columns, String[] vals) {
		...
		for (int i = 0; i < columns.length; i++) {
			...
			col.setName(columns[i].trim().getBytes());
			col.setValue(vals[i].trim().getBytes());
	...
	private static SlicePredicate makeSlicePred(String[] columns) {
		...
		for (String col : columns) 
			cols.add(col.trim().getBytes());
		slicePred.setColumn_names(cols);
		return slicePred;
	}

	private final static String SEP_PAT = ",";
